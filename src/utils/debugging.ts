...
export function useMapDebugger(componentName: string) {
  ...
  return {
    logRender: (props?: Record<string, unknown>, state?: Record<string, unknown>) =>
...
export function withMapDebugging<P extends object>(
  ...
  return function DebuggedComponent(props: P) {
    const { logRender, logError, trackPerformance } = useMapDebugger(displayName)
...
export const devTools = {
   ...
   */
  debugGeographyLoading: (url: string, status: "start" | "success" | "error", data?: unknown) => {
    if (typeof process !== "undefined" && process.env.NODE_ENV !== "production") {
      const ownerStack = captureOwnerStack()
      // eslint-disable-next-line no-console
...
if (typeof process !== "undefined" && process.env.NODE_ENV !== "production") {
  ;(globalThis as typeof globalThis & { __MAP_DEBUGGER__?: MapDebugger }).__MAP_DEBUGGER__ =
    mapDebugger
}
