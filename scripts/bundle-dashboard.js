...
class BundleDashboard {
  ...
  loadHistoricalReports(limit = 10) {
    ...
    const reportFiles = readdirSync(this.reportsDir)
      ...
      .sort((a, b) => {
        const timeA = parseInt(a.match(/bundle-analysis-(\d+)\.json/)?.[1] || '0')
        const timeB = parseInt(b.match(/bundle-analysis-(\d+)\.json/)?.[1] || '0')
        return timeB - timeA // Most recent first
      })
      .slice(0, limit)

    this.historicalReports = reportFiles.map(file => {
      try {
    ...
    const comparison = {
      ...
      totalSize: {
        raw: {
          ...
          change: newReport.summary.totalSizes.raw - oldReport.summary.totalSizes.raw,
          changePercent: ((newReport.summary.totalSizes.raw - oldReport.summary.totalSizes.raw) / oldReport.summary.totalSizes.raw * 100).toFixed(2),
        },
        gzip: {
          old: oldReport.summary.totalSizes.gzip,
          new: newReport.summary.totalSizes.gzip,
          change: newReport.summary.totalSizes.gzip - oldReport.summary.totalSizes.gzip,
          changePercent: ((newReport.summary.totalSizes.gzip - oldReport.summary.totalSizes.gzip) / oldReport.summary.totalSizes.gzip * 100).toFixed(2),
        },
      },
  ...
  compareBundles(oldBundles, newBundles) {
    ...
    newBundles.forEach(newBundle => {
      ...
      changes.push({
        ...
        gzipChangePercent: ((gzipChange / oldBundle.sizes.gzip) * 100).toFixed(2),
        trend: rawChange > 0 ? 'increased' : rawChange < 0 ? 'decreased' : 'stable',
      })
    })
    ...
    Object.keys(latest.react19Optimizations).forEach(key => {
      ...
      if (previousOpt) {
        progress[key] = {
          ...
          improvement: parseFloat(latestOpt.completionRate) - parseFloat(previousOpt.completionRate),
    ...
    this.currentReport.bundles.forEach(bundle => {
      ...
      if (!bundle.compliance.gzip) {
        recommendations.push({
          ...
          suggestion: 'Review compression settings and consider additional minification',
        })
      }
    })

    // Check React 19 optimization status
    Object.entries(this.currentReport.react19Optimizations).forEach(([key, optimization]) => {
  ...
  printDashboard() {
    ...
    console.log(`📦 Total Size: ${this.currentReport.summary.totalSizes.rawFormatted} raw, ${this.currentReport.summary.totalSizes.gzipFormatted} gzip`)
    console.log(`✅ Compliance: ${this.currentReport.summary.compliantBundles}/${this.currentReport.summary.existingBundles} bundles\n`)
    ...
    this.currentReport.bundles.forEach(bundle => {
      ...
      const complianceIcon = bundle.compliance.raw && bundle.compliance.gzip ? '✅' : '⚠️'
      console.log(`${complianceIcon} ${bundle.name}`)
      console.log(`   Raw: ${bundle.sizes.rawFormatted} (${bundle.utilization.raw}% of limit)`)
      console.log(`   Gzip: ${bundle.sizes.gzipFormatted} (${bundle.utilization.gzip}% of limit)`)
    })

    // React 19 optimizations
    console.log('\n🚀 React 19 Optimizations')
    Object.entries(this.currentReport.react19Optimizations).forEach(([key, opt]) => {
      const statusIcon = opt.status === 'complete' ? '✅' : '🔄'
      console.log(`${statusIcon} ${opt.name}: ${opt.completionRate}% complete (${opt.expectedSavings} potential savings)`)
    })
    ...
    if (this.historicalReports.length > 1) {
      ...
      console.log(`${trendIcon} Size change: ${shortTerm.totalSize.raw.changePercent}% raw, ${shortTerm.totalSize.gzip.changePercent}% gzip`)
    }
