...
const OPTIMIZATION_STRATEGIES = {
  ...
  stripDevelopmentCode: {
    ...
    description:
      'Remove console.log, debugging utilities, and development-only features',
    patterns: [
  ...
  optimizeImports: {
    ...
    description:
      'Use specific imports instead of barrel exports to enable better tree shaking',
    patterns: [
  ...
  try {
    ...
    const potentialIssues = [
      ...
      },
      {
        ...
        message:
          'Complex map functions detected - consider extracting to separate functions',
        severity: 'medium',
      },
      {
        pattern: /useEffect\(\s*\(\s*\)\s*=>\s*{[\s\S]{200,}?}\s*,/g,
        message:
          'Large useEffect detected - consider splitting into smaller effects',
        severity: 'medium',
      },
    ];
...
function generateOptimizationReport() {
  ...
  const analyses = sourceFiles
    .map((file) => analyzeCodePatterns(join(projectRoot, file)))
...
function printOptimizationReport(report) {
  ...
  console.log(
    `📦 Total source size: ${(report.totalSize / 1024).toFixed(2)} KB\n`,
  );
  ...
  report.recommendations.forEach((rec, index) => {
    ...
    console.log(
      `${index + 1}. ${priorityIcon} ${rec.action} (${rec.priority} priority)`,
    );
  ...
  if (totalPotentialSavings > 0) {
    console.log(
      `💡 Estimated total bundle size reduction: ${totalPotentialSavings.toFixed(1)}%`,
    );
  }
}
